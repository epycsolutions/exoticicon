...
describe('Assets utilities', () => {
    ...
    describe('loadPaths', () => {
        ...
        it('resolved an Array of the correct filePaths within the given directory', async () => {
    ...
    describe('loadAssets', () => {
        it('resolved a key - value map of assets with expected properties', async () => {
            expect(
                await loadAssets({
                    ...DEFAULT_OPTIONS,
                    inputDir: './valid',
        ...
        it('generates icon IDs correctly when called with a custom `getIconId` function', async () => {
            const getIconId: GetIconIdFn = jest.fn(
        ...
        it('rejects correctly if `getIconId` resolved the same `key` while processing different assets', async () => {
    ...
    describe('writeAssets', () => {
        it('calls `fs.writeFile` for each given asset with correctly formed filePath and content', async () => {
            await writeAssets(
                ...
                { name: 'base-name', outputDir: '/dev/null' } as any
            );
        ...
        it('return an object containing information on written assets', async () => {
            expect(
                await writeAssets(
                    { svg: '::svg-content::', foo: '::foo-content::' } as any,
                    {
