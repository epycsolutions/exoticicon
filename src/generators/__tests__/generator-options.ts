...
describe('Font generator options', () => {
    ...
    test('`prefillOptions` correctly extends default value for each type and prefills missing ones', () => {
        expect(
            prefillOptions<any, any>(
                ['html', 'ttf', 'eot', 'css', 'foo'],
                {
                    html: { a: 'a', c: 'c', baseVal: 'custom' },
                    ttf: { foo: 'bar' }
                },
                (key) => ({ baseVal: key })
            )
    ...
    test('`getGeneratorOptions` produces usable font generator options including given `assets` and sanitised `formatOptions`', () => {
        ...
        expect(Object.keys(generatorOptions.formatOptions)).toHaveLength(Object.keys(ASSET_TYPES).length)
        expect(Object.keys(generatorOptions.templates)).toHaveLength(ASSET_TYPES_WITH_TEMPLATE.length)
    })
